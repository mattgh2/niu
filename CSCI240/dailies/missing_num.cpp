...
int missing_number(int a[], int l){
  int missingNum, min_pos;
  bool found = false;
  // sort the list
  for (int i = 0; i < l - 1; i++){
    min_pos = i;
  ...
  if (!found){ // missing number must be 0
    if (a[0] != 0){
      missingNum = 0; 
    }
    else{ // missing number must be the last possible value 
      missingNum = l; 
    }
  }
  return missingNum;
