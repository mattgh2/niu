...
// each element in the new array is equal to the product of the corresponding value from array source * the sum of all the elements in source except the corresponding value
// let arr = {2,-4,10}
// result[0] = (-4 + 10) * 2 = 12
// result[1] = (2 + 10) * -4 = -48
int prod_sum(int source[], int result[], int num_values){
  ...
  int sum =0;
  for (int i = 0; i < num_values; i++){ // getting sum of all elements
    sum+= source[i];
  }
  for (int j = 0; j < num_values; j++){
    result[j] = (sum - source[j]) * source[j];
  }
  if (num_values < 2){
    return 0;
  }
